...
export function ConfiguratorSection() {
  ...
  return (
    <section id="configurator" className="relative flex flex-col w-full h-full min-h-screen mx-auto px-4 sm:px-6 lg:px-8 justify-between py-24 overflow-hidden bg-background-dark">
      ...
      {/* Top right price */}
      <div className="w-full flex items-start z-10 justify-end max-w-7xl mx-auto absolute top-24 right-8 md:right-20">
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10 max-w-7xl mx-auto px-0 md:px-12 mt-auto">
        {/* Colors Panel */}
        <div className="glass-effect rounded-2xl shadow-soft transition-all duration-300 flex flex-col items-center p-6">
          ...
          <div className="w-full flex items-center justify-between">
            <button
              ...
              className={cn("w-8 h-8 flex items-center justify-center text-slate-400 hover:text-primary transition-colors", colorPage === 0 && "opacity-0 cursor-default")}
            >
            ...
            <div className="grid grid-cols-3 gap-8">
              {COLORS_PAGES[colorPage].map((color) => (
                <button
                  ...
                  className={cn(
                    "group relative w-12 h-12 rounded-full shadow-sm transition-transform hover:scale-110 focus:outline-none",
                    color.bg,
                    selectedColor === color.id && "ring-2 ring-primary ring-offset-2 ring-offset-background-dark"
                  )}
                >
            ...
            <button
              onClick={() => setColorPage(p => Math.min(COLORS_PAGES.length - 1, p + 1))}
              className={cn("w-8 h-8 flex items-center justify-center text-slate-400 hover:text-primary transition-colors", colorPage === COLORS_PAGES.length - 1 && "opacity-0 cursor-default")}
            >
              <MaterialIcon name="chevron_right" className="text-lg" />
            </button>
          </div>
          <p className="text-sm font-medium text-white mt-4 h-5">
            {activeColorName}
          </p>
        </div>
        ...
        <div className="glass-effect rounded-2xl shadow-soft transition-all duration-300 flex flex-col items-center p-6">
          ...
          <div className="w-full flex items-center justify-between">
            <button
              onClick={() => setWheelPage(p => Math.max(0, p - 1))}
              className={cn("w-8 h-8 flex items-center justify-center text-slate-400 hover:text-primary transition-colors", wheelPage === 0 && "opacity-0 cursor-default")}
            >
              <MaterialIcon name="chevron_left" className="text-lg" />
            </button>
            <div className="grid grid-cols-3 items-center gap-8">
              {WHEELS_PAGES[wheelPage].map((wheel) => (
                <div
                  ...
                  className={cn(
                    "w-14 h-14 rounded-full cursor-pointer transition-all flex items-center justify-center overflow-hidden",
                    selectedWheel === wheel.id ? "bg-slate-800 shadow-sm ring-2 ring-primary ring-offset-2 ring-offset-background-dark" : "p-1 hover:bg-black/5"
                  )}
                >
                  <img
                    src={wheel.src}
            ...
            <button
              onClick={() => setWheelPage(p => Math.min(WHEELS_PAGES.length - 1, p + 1))}
              className={cn("w-8 h-8 flex items-center justify-center text-slate-400 hover:text-primary transition-colors", wheelPage === WHEELS_PAGES.length - 1 && "opacity-0 cursor-default")}
            >
              <MaterialIcon name="chevron_right" className="text-lg" />
            </button>
          </div>
        ...
        <div className="glass-effect rounded-2xl shadow-soft transition-all duration-300 flex flex-col items-center p-6">
          ...
          <div className="w-full flex items-center justify-between">
            <button
              ...
              className={cn("w-8 h-8 flex items-center justify-center text-slate-400 hover:text-primary transition-colors", detailPage === 0 && "opacity-0 cursor-default")}
            >
              <MaterialIcon name="chevron_left" className="text-lg" />
            </button>
            <div className="grid grid-cols-3 items-center gap-8">
              {DETAILS_PAGES[detailPage].map((detail) => (
                <div
                  ...
                  className={cn(
                    "w-14 h-14 rounded-full overflow-hidden cursor-pointer transition-all hover:scale-105",
                    selectedDetail === detail.id ? "shadow-sm ring-2 ring-primary ring-offset-2 ring-offset-background-dark" : "opacity-50 hover:opacity-100"
                  )}
                >
                  <img
                    src={detail.src}
            ...
            <button
              onClick={() => setDetailPage(p => Math.min(DETAILS_PAGES.length - 1, p + 1))}
              className={cn("w-8 h-8 flex items-center justify-center text-slate-400 hover:text-primary transition-colors", detailPage === DETAILS_PAGES.length - 1 && "opacity-0 cursor-default")}
            >
              <MaterialIcon name="chevron_right" className="text-lg" />
            </button>
          </div>
